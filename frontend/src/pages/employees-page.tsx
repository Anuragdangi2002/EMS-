...
const name = (field: string) =>
  required(field).regex(
    /^[\p{L}\s'-]+$/u,
    `${field} cannot contain numbers or symbols`,
  );
const today = () =>
  new Date(
...
const employeeSchema = z.object({
  ...
  joiningDate: required("Joining date").refine(
    (value) =>
      !Number.isNaN(asLocalDate(value).getTime()) &&
      asLocalDate(value) <= today(),
  ...
  postalCode: required("Postal code").regex(
...
type EmployeeWithContact = Employee & {
  email?: string;
  phone?: string | null;
  user?: Pick<User, "email" | "phone" | "role">;
};
...
function EmployeeModal({
  ...
  const saving = useMutation({
    mutationFn: async (values: EmployeeForm) => {
      ...
      const leaveFields = {
        allocatedLeaves: values.allocatedLeaves
          ? parseFloat(values.allocatedLeaves)
          : 20,
        leaveBalance: values.leaveBalance
          ? parseFloat(values.leaveBalance)
          : 20,
      };
      if (isEdit) {
        ...
        const accountResponse = await api.post<ApiEnvelope<{ user: User }>>(
    ...
    onError: (error) =>
      toast.error(
        messageFrom(
          error,
          isEdit ? "Unable to update employee" : "Unable to create employee",
        ),
      ),
  });
  ...
  const selectClass =
    "mt-1.5 w-full rounded-lg border border-slate-300 bg-white px-3 py-2.5 text-sm outline-none focus:border-blue-600 focus:ring-3 focus:ring-blue-100";
  ...
  const field = (name: keyof EmployeeForm, label: string, type = "text") => (
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 p-4 backdrop-blur-sm overflow-y-auto">
      <form
        onSubmit={form.handleSubmit((values) => saving.mutate(values))}
        className="relative w-full max-w-4xl rounded-xl bg-white shadow-2xl transition-all border border-slate-100 max-h-[90vh] flex flex-col"
      >
        <div className="flex items-center justify-between border-b px-6 py-4">
          <div>
            <h2 className="font-semibold text-slate-955">
              {isEdit ? "Edit employee" : "Add employee"}
            </h2>
            <p className="text-xs text-slate-500">
              {isEdit
                ? "Update employee profile details."
                : "Creates an employee login first, then their employee profile."}
            </p>
          </div>
          <button
            type="button"
          ...
          <div>
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              ...
              >
                ...
                >
                  {["FULL_TIME", "PART_TIME", "INTERN", "CONTRACT"].map(
                    (value) => (
              ...
              <FormField label="Upload profile image" error={imageError}>
                ...
                {profileImageUrl && (
                  <img
                    src={profileImageUrl}
                    alt="Profile preview"
                    className="mt-3 size-20 rounded-full border border-slate-200 object-cover"
                  />
                )}
              </FormField>
            </div>
...
function EmployeeDetailsModal({
  ...
  const details = useQuery({
    ...
    queryFn: () =>
      employeeService.get(initialEmployee!.id) as Promise<EmployeeWithContact>,
    enabled: !!initialEmployee?.id,
  });
  const employee = details.data ?? initialEmployee;
  const department =
    departments.data?.find((item) => item.id === employee?.departmentId)
      ?.name ?? employee?.departmentId;

  const managerName = useMemo(() => {
    if (!employee?.managerId) return "None";
    const mgr = employeesQuery.data?.find(
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 p-4 backdrop-blur-sm">
        <div className="w-full max-w-md rounded-xl bg-white p-6 shadow-2xl border border-slate-100 text-left">
          <p className="text-sm font-semibold text-red-600">
            {messageFrom(details.error, "Unable to load employee details")}
          </p>
  ...
  const values: Array<[string, React.ReactNode]> = [
    ...
    [
      "Leave balance",
      `${employee.leaveBalance ?? 20} / ${employee.allocatedLeaves ?? 20} days`,
    ],
    ...
    [
      "Status",
      <Badge tone={employee.isActive ? "green" : "gray"}>
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1">
            ...
            <p className="mt-2 text-sm text-slate-500">
              Are you sure you want to deactivate this employee? This will
              suspend their account access and mark their profile as inactive.
            </p>
...
export function EmployeesPage() {
  ...
  const [editingEmployee, setEditingEmployee] = useState<
    EmployeeWithContact | null | undefined
  >();
  const [selectedEmployee, setSelectedEmployee] =
    useState<EmployeeWithContact>();
  ...
  const columns: ColumnDef<Employee>[] = [
      ...
      cell: (context) => (
        <div className="text-left">
          <p className="font-medium text-slate-900">
            {context.row.original.firstName} {context.row.original.lastName}
          </p>
          <p className="text-xs text-slate-500">
            {context.row.original.employeeCode}
          </p>
        </div>
      ),
    },
    { accessorKey: "designation", header: "Designation" },
    {
      ...
      header: "Type",
      cell: (context) => title(context.getValue<string>()),
    },
    {
      accessorKey: "joiningDate",
      header: "Joined",
      cell: (context) => date(context.getValue<string>()),
    },
    {
      accessorKey: "isActive",
      header: "Status",
      cell: (context) => (
        <Badge tone={context.getValue() ? "green" : "gray"}>
          {context.getValue() ? "Active" : "Inactive"}
        </Badge>
      ),
    },
    {
      ...
      cell: (context) => (
        <div className="flex gap-1">
          <button
            ...
            onClick={() => setSelectedEmployee(context.row.original)}
          ...
          {hasRole("ADMIN", "HR") && (
            <button
              ...
              onClick={() => setEditingEmployee(context.row.original)}
              className="cursor-pointer rounded p-1.5 text-slate-500 hover:bg-slate-100 hover:text-blue-700"
            >
          ...
          {hasRole("ADMIN", "HR") && (
            <button
              ...
              onClick={() => setDeactivatingEmployee(context.row.original)}
              className="rounded p-1.5 text-slate-500 hover:bg-red-50 hover:text-red-700 disabled:cursor-not-allowed disabled:opacity-50"
            >
  ...
  return (
    <>
      <PageHeader
        title="Employees"
        description="Employee records and workforce directory."
        actions={
          hasRole("ADMIN", "HR") ? (
      ...
      {editingEmployee && (
        <EmployeeModal
          ...
          close={() => setEditingEmployee(undefined)}
        />
      )}
      {selectedEmployee && (
        <EmployeeDetailsModal
          employee={selectedEmployee}
          close={() => setSelectedEmployee(undefined)}
        />
      )}
